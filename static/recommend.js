...
var myAPI = '78720ad51fd22cc69b2babb6a3e4d1b4'  // global string to be consistent with future usages elsewhere
$(function() {
  $('#movie_list').css('display','none'); // autosearch movielist initially hidden
  ...
  const inputHandler = function(e) {
    $('#movie_list').css('display','block'); //movielist is visible
    if(e.target.value==""){
...
function load_details(my_api_key,search,isQuerySearch){
  if(isQuerySearch) {
    ...
    url = 'https://api.themoviedb.org/3/movie/' + search + '?api_key='+my_api_key; //Query For Details with movie id
  }
  $.ajax({
    type: 'GET',
    url:url,
    ...
    success: function(movie){
      if(!isQuerySearch) { // single movie result
        $("#loader").fadeIn();
...
function gfg(n) {
// To access the stars
let rating = document.getElementsByClassName("star");
...
for (let i = 0; i < n; i++) {
    ...
    rating[i].className = "star " + cls;
}
output.innerText = "Your Rating is: " + n + "/5";
...
let i = 0;
while (i < 5) {
    rating[i].className = "star";
    i++;
}
}

function sendRatingToServer(n) {
  let xhr = new XMLHttpRequest();
  // Set up the request
  xhr.open("POST", "/store-rating", true);
  xhr.setRequestHeader("Content-Type", "application/json");
  // Define the data to be sent
  let data = JSON.stringify({rating: n });
